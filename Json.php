...
class Json implements JsonInterface {
	...
	{
		if(!is_null($key)) return $this->select($key);
		return $this->data;
	}
	 ...
	 * @return string
	 */
	function output(string $key): ?string 
	{
		$arr = $this->select($key);
		if($get = self::encodeData($arr)) return htmlentities($get);
		return NULL;
	}
	...
	public function validate(): void 
	{
		 switch(self::error()) {
	        case JSON_ERROR_DEPTH:
	            throw new \Exception('The maximum stack depth has been exceeded', JSON_ERROR_DEPTH);
	        break;
	        case JSON_ERROR_STATE_MISMATCH:
	            throw new \Exception('Invalid or malformed JSON', JSON_ERROR_STATE_MISMATCH);
	        break;
	        case JSON_ERROR_CTRL_CHAR:
	            throw new \Exception('Control character error, possibly incorrectly encoded', JSON_ERROR_CTRL_CHAR);
	        break;
	        case JSON_ERROR_SYNTAX:
	            throw new \Exception('Syntax error', JSON_ERROR_SYNTAX);
	        break;
	        case JSON_ERROR_UTF8:
	            throw new \Exception('Malformed UTF-8 characters, possibly incorrectly encoded', JSON_ERROR_UTF8);
	        break;
	        case JSON_ERROR_RECURSION:
	            throw new \Exception('One or more recursive references in the value to be encoded', JSON_ERROR_RECURSION);
	        break;
	        case JSON_ERROR_INF_OR_NAN:
	            throw new \Exception('One or more NAN or INF values in the value to be encoded', JSON_ERROR_INF_OR_NAN);
	        break;
	        case JSON_ERROR_UNSUPPORTED_TYPE:
	            throw new \Exception('A value of a type that cannot be encoded was given', JSON_ERROR_UNSUPPORTED_TYPE);
	        break;
	        case JSON_ERROR_INVALID_PROPERTY_NAME:
	            throw new \Exception('A property name that cannot be encoded was given', JSON_ERROR_INVALID_PROPERTY_NAME);
	        break;
	        case JSON_ERROR_UTF16:
	            throw new \Exception('Malformed UTF-16 characters, possibly incorrectly encoded', JSON_ERROR_UTF16);
	        break;
	    }
	}
